...
def toggle_current_mission(func):
    def _toggle_current_mission(*args):
        func(*args)
        args[0]._toggle_mission()
    return _toggle_current_mission

def place_mission(func):
    def _place_missions(*args):
        mission = func(*args)
        self = args[0]

        # if empty list
        if isinstance(self.content[-1], urwid.Text):
            self.content.pop()
            self.content.append(urwid.AttrMap(RealmWidget(mission.realm), None, 'reveal focus'))
            self.content.append(urwid.AttrMap(MissionWidget(mission), None, 'reveal focus'))
            self.go_down()

        # find the context where I want to add this mission
        else:
            it = 0
            while it < len(self.content) and\
                  not (isinstance(self.content[it].original_widget, RealmWidget) and\
                       self.content[it].original_widget.realm == mission.realm):
                it += 1
            it += 1

            # if I found it, append to it end
            if it <= len(self.content):
                while not isinstance(self.content[it].original_widget, urwid.Divider) and it < len(self.content) - 1:
                    it += 1
                if it == len(self.content) - 1:
                    it += 1
                self.content.insert(it, urwid.AttrMap(MissionWidget(mission), None, 'reveal focus'))
            else:
                louie.send("update_main")


    return _place_missions

def place_due_mission(func):
    def _place_due_missions(*args):
        mission = func(*args)
        self = args[0]

        current_todo_due_date = mission.due
        due_date = None
        if current_todo_due_date:
            for i, j in ((1,  "FOR TODAY"), (4,  "FOR IN 3 DAYS"), (8,  "FOR THIS WEEK")):
        ...
        # find the context where I want to add this mission
            ...
            it = 0
            while it < len(self.content) and\
                  not (isinstance(self.content[it].original_widget, SeparationWidget) and\
                       self.content[it].original_widget.text == due_date):
                it += 1
            it += 1
        ...
        current = self.frame.get_body().get_focus()[0].original_widget
        if isinstance(current, urwid.Divider):
            self.position -= 1
            self.content.set_focus(self.position)
        elif isinstance(current, SeparationWidget):
            self.position += 1
            self.content.set_focus(self.position)

    return _place_due_missions

def append_mission_to_current_group(func):
    def _append_mission_to_current_group(*args):
        mission = func(*args)
        self = args[0]
    ...
    return _append_mission_to_current_group

def add_to_completed_task(func):
    def _add_to_completed_task(*args):
        self = args[0]
        task = func(*args)
        ...
        if isinstance(self.content[-1], urwid.Text):
            ...
            self.content.append(urwid.AttrMap(MissionWidget(task), None, 'reveal focus'))
            self.go_down()

        # if last completed missions already exist
        elif isinstance(self.content[-1].original_widget, MissionWidget) and\
           self.content[-1].original_widget.mission.completed:
            a = len(self.content) - 1
            while not isinstance(self.content[a].original_widget, SeparationWidget):
                a -= 1
            self.content.insert(a + 1, urwid.AttrMap(MissionWidget(task), None, 'reveal focus'))

        # if last completed missions doesn't exist
        else:
            self.content.append(urwid.AttrMap(urwid.Divider(" "), None, 'reveal focus'))
            self.content.append(urwid.AttrMap(SeparationWidget("LAST COMPLETED MISSIONS"), None, 'reveal focus'))
            self.content.append(urwid.AttrMap(MissionWidget(task), None, 'reveal focus'))

        # if < 5 last completed missions, remove the last one
...
def remove_current_item(func):
    def _remove_current_item(*args):
        ...
        ## at the end of the list
            ...
            self.content.set_focus(self.position)
            if isinstance(self.content[self.position].original_widget, SeparationWidget):
                # clean useless stuff
                self.content.pop()
                self.content.pop()
                ## go down if not at bottom, else go to one
                self.position = self.position - 2 if (self.position - 2) > 0 else 1
                ## nothing in the list ? go to 0
                if len(self.content) == 1:
                    self.position = 0
                self.content.set_focus(self.position)

        elif not isinstance(self.content[self.position].original_widget, urwid.Divider):
            self.go_down()
            self.content.set_focus(self.position)

        ## have remove everything from a realm or a "for in x time"
        if isinstance(self.content[self.position].original_widget, urwid.Divider)\
             and isinstance(self.content[self.position - 1].original_widget, SeparationWidget):
            # clean useless stuff
        ...
        if removed.original_widget.mission.completed:
            self.refill_last_completed_missions()

        if len(self.content) != 1 and isinstance(self._get_current_widget(), urwid.Divider):
            self.position -= 1
            self.content.set_focus(self.position)
